...
if !node_type == operator or $NODE_TYPE == master-operator then process !local_scripts/database/configure_dbms_operator.al
if (!node_type == operator or $NODE_TYPE == master-operator) and !enable_nosql == true then process !local_scripts/database/configure_dbms_nosql.al
...
if !node_type == operator or !node_type == publisher or $NODE_TYPE == master-operator or $NODE_TYPE == master-publisher  then process !local_scripts/database/configure_dbms_almgm.al
