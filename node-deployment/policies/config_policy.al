...
<set policy new_policy [config][script] = [
    ...
    "run streamer",
    "if !local_blockchain == false then run publisher where compress_json=!compress_file and compress_sql=!compress_file and blockchain=!platform and dbms_name=!dbms_file_location and table_name=!table_file_location",
    "if !local_blockchain == true then run publisher where compress_json=!compress_file and compress_sql=!compress_file and master=!ledger_conn and dbms_name=!dbms_file_location and table_name=!table_file_location",
    "if !monitor_nodes == true then process !anylog_path/deployment-scripts/demo-scripts/monitoring_policy.al",
...
<set policy new_policy [config][script] = [
    ...
    "if !operator_id and !local_blockchain == false then run operator where create_table=!create_table and update_tsd_info=!update_tsd_info and compress_json=!compress_file and compress_sql=!compress_sql and archive_json=!archive and archive_sql=!archive_sql and blockchain=!platform and policy=!operator_id and threads=!operator_threads",
    "if !operator_id and !local_blockchain == true then run operator where create_table=!create_table and update_tsd_info=!update_tsd_info and compress_json=!compress_file and compress_sql=!compress_sql and archive_json=!archive and archive_sql=!archive_sql and master=!ledger_conn and policy=!operator_id and threads=!operator_threads",
